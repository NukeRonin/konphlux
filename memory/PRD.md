...
## Implemented
### 2026-06 (LAYOUT FIX: Vault/Retro buttons now visible + Profile Creations Grid · Mutual Friends List · Notification Deep-Links; user self-testing)
- ✅ **ROOT CAUSE of "missing buttons"**: the category/quick buttons DID exist but lived in a **horizontal scroll strip** that only showed the first ~3 — the rest (incl. the new ones) were off-screen sideways. Converted both to **wrapping grids (flexWrap)** so every button is visible at once. Verified on preview: all 24 Vault chips wrap into view (Hints & Walkthrus, TV Show Recommendations, etc.) and Retrospections shows Nearby/Status/**Temporary Closures**/**Closing Soon**/Favorites/For Sale as tiles. Vault grid moved to the top of the content ScrollView; Retro `quick` row is now a flexWrap View.
- ✅ **Profile Creations Grid**: `GET /users/{id}/creations` returns the member's Frankenstein creations + published courses (grid) and the reviews they've written (list). Rendered on /u/[id] as a 2-col creation grid + reviews section, each tappable to open the item/course.
- ✅ **Mutual Friends List**: `GET /users/{id}/mutual` returns shared friend cards; the "N mutual friends ›" line on a profile now opens a bottom-sheet listing them, each tappable to their profile.
- ✅ **Notification Deep-Links**: notifications screen routes by type — `friend_request`/`friend_accept` → `/u/{listing_id}`, `retro_reopen` → business page, else product. Friend notifications store the other user's id in listing_id.
- Files: backend/server.py (user_mutual + user_creations endpoints); frontend app/vault/index.tsx (wrapping catGrid), app/retrospections/index.tsx (wrapping quick grid), app/u/[id].tsx (ScrollView + creations grid + reviews + mutual modal), app/notifications.tsx (type-based routing + icons), src/api/client.ts (ProfileCreation/ProfileReview types, userMutual/userCreations).
- Verified: preview screenshots confirm both button grids render fully; backend smoke (creations/reviews + mutual endpoints 200). Lint clean, iOS entry bundle 200. Backend + Expo restarted.
