...
export const api = {
  ...
  libraryStats: () => request<{ hours_this_month: number; minutes_this_month: number; total_hours: number; books_finished_this_month: number; books_finished_total: number; streak_days: number; listened_today: boolean; monthly_goal: number }>("/library/stats"),
  librarySetGoal: (goal: number) => request<{ monthly_goal: number }>("/library/goal", { method: "POST", body: JSON.stringify({ goal }) }),
